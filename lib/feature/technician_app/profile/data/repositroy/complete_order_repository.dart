...
class CompleteOrderRepository extends BaseCompletedOrdersRepository {
  ...
  Future<Either<Failure, List<MyOrderDataEntity>>> calenderOrders(
      {required MyOrdersCalenderParameter myOrdersCalenderParameter}) async {
    try {
      return Right(
          await baseCompletedOrderDataSource.calenderOrders(myOrdersCalenderParameter: myOrdersCalenderParameter));
    } on Failure catch (ex) {
      return Left(ServerFailure(code: ex.code, message: ex.message));
    }
  }
